<template>
    <div>
        <div id="conversationDrop" class="app--conversation--drop--wrap" v-cloak @drop.prevent="addFile" @dragover.prevent @dragleave="moveOut">Перетащите сюда фотографии, чтобы отправить их</div>
        <div class="app--conversation--sendForm" style="display:flex; flex-direction: column; position:relative;">
            ...
            <AppTopicBar :conversation-id="conversationId"></AppTopicBar>

            <div class="app--conversation--sendForm" style="display:flex; flex-direction: column;">
                <div class="app--conversation--sendForm">

                    <div style="display: flex;align-self: flex-start;justify-content: center;align-items: center;margin-top: 25px;">
                        ...
                        <div class="app--conversation--sendForm__button app--conversation--sendForm__button__attach" @click="$refs.attachFile.click()">
                            <img src="/icons/attachment.svg" alt="">
                        </div>
                        <input
                                id="file"
                                ref="attachFile"
                    ...
                    <div style="display:flex; width:100%;">
                        <div style="position:relative; width:95%;" id="messageWrapper">
                            <div class="app--conversation--sendForm__recorder__wrapper" v-if="isRecording">
                            ...
                            <span class="app--conversation--sendForm__message__button app--conversation--sendForm__message__button__microphone">
                            <img v-if="!isRecording" src="/icons/microphone.svg" alt="" id="record"
                            ...
                            <textarea
                                    ...
                                    v-on:keyup.ctrl.13="addLine($event)"
                                    v-on:keyup.enter.exact="send(sendType)"
                            ></textarea>

                            <div id="attachments--wrapper"  class="app--conversation--sendForm__attachments__wrapper">
                                <div v-for="(item, index) in files" style="position:relative">
                                    <i class="fa fa-times" @click="attachmentDelete(index)"></i>
                                    <img class="preview" :src="item.thumb" />
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    //
    import Recorder from "@components/lib/recorder";
    ...
    export default {
        ...
        watch: {
            ...
            conversationId(v) {
               //console.log('current', this.conversation);
                ...
                if (this.overlayChat) {
                    let elem = this.$refs.conversationField;
        ...
        computed: {
            ...
            validateSendMessage() {
                ...
                if (this.$store.getters["chat/message/messages"] &&
                    this.$store.getters["chat/message/messages"][this.conversationId]) {

                    // проверка на строку из пробелов и вложения
                    if (/\S/.test(this.message) || this.files.length || this.message.trim() != '' || this.message.replace('\n', '').trim() != '' || (this.quotedMessage && this.quotedMessage.id) ) {
                        return true;
                    }
                }
                this.clearForm();
        ...
        methods: {
            changeCurrent(){
                this.$store.commit('chat/conversation/setCurrentConversationMessage', this.message);
            },
            ...
            methodsNotAvailable(){
                this.$notify({
                    message: "В данный момент ведутся технические работы связанные с этим функционалом",
                    type: "warning"
                });
            },
            moveOut(){
            ...
            validateFile() {
                ...
                for(let i = 0; i < this.$refs.attachFile.files.length; i++){
                    let attachFile = this.$refs.attachFile.files[i];
                    if (attachFile) {
            ...
            async previewAttachFile(file, i) {
                ...
                let binary = await this.readFile(file);
                this.attachFile = btoa(binary);
                let thumb = await this.getThumb(file);
                const attach = {
        ...
        mounted() {
            document.getElementById('conversationWrap').addEventListener('dragover', function(){
                document.getElementById('conversationDrop').style.width = document.getElementById('conversationWrap').offsetWidth + 'px';
