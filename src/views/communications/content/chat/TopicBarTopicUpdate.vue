...
import AppInput from '../../../../template/form/Input'
...
export default {
    ...
    computed: {
        ...
        topicOld () {
            ...
            let topic = topics.filter(item => item.id == this.topicId)
            if (topic.length) {
                return topic[0]
            }
            return {}
        ...
        nameErrors () {
            ...
            !this.$v.topic.name.required && errors.push('Поле "Название темы" обязательно')
            !this.$v.topic.name.maxLength && errors.push('Поле "Название темы" не может быть более 100 символов')
            this.nameExist && errors.push('Тема с таким названием уже существует')
            return errors
        },
        validateTopic () {
            if (this.$v.topic.name.required && this.$v.topic.name.maxLength
                && this.checkRecipients() && !this.nameExist
                && JSON.stringify(this.topic) != JSON.stringify(this.topicOld)) return true
            return false
        }
    },
    methods: {
        ...
        checkRecipients () {
            let recipients = this.topic.members.filter(item => item.id != this.chatUser.id)
        ...
        isRecipient (id) {
            if (!this.isEmptyObject(this.topic)) {
                let check = this.topic.members.filter(item => item.id == id)
                if (check.length) return true
            }
            return false
        },
        getCanWriteClass (id) {
            let recipient = this.topic.members.filter(item => item.id == id)
            if (recipient.length) {
        ...
        setCanWrite (id) {
            ...
            if (index !== null) {
                if (this.topic.members[index]) {
                    this.topic.members[index].can_write = !this.topic.members[index].can_write
                }
            }
            else {
                this.topic.members.push({
