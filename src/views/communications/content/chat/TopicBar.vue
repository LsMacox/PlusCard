        ...
        >
            ...
            >
                ...
                <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item
                            ...
                            :key="i"
                            @click.native="setTopic(item.id)"
                    >{{ item.name }}</el-dropdown-item>
                    <el-dropdown-item
                            @click.native="setTopic(0)"
                    ><span style="color: #24D3ED;">Другая или новая тема</span></el-dropdown-item>
                </el-dropdown-menu>
            </el-dropdown>
            ...
            >
                ...
                <el-dropdown-menu slot="dropdown">
                    ...
                    >{{ item.name }}</el-dropdown-item>
                    <el-dropdown-item
                            @click.native="setGroup(0)"
                    ><span style="color: #24D3ED;">Другая или новая группа</span></el-dropdown-item>
                </el-dropdown-menu>
            </el-dropdown>
...
export default {
    ...
    computed: {
        ...
        lastTopic () {
            ...
            topics = this.topics;
            messages = this.$store.getters['chat/message/messages'][this.conversationId]
            let key = null
            ...
            if (key) {
                while (true) {
                    ...
                    if (message && message.sender_id == this.chatUser.id && topics.filter(item => item.id == message.topic_id).length > 0) {
                            return {
    ...
    methods: {
        ...
        getTopicName () {
            let topic = this.topics.filter(item => item.id == this.selectedTopicId)
            if (topic.length) {
                return topic[0].name
            }
            return ''
        ...
        setGroup (id) {
            if (id === 0) this.openDialogGroups()
                ...
                let group = this.groups.filter(item => item.id == id)
                if (group.length) {
        ...
        getGroupName () {
            let group = this.groups.filter(item => item.id == this.selectedGroupId)
            if (group.length) {
                return group[0].name
            }
            return ''
