<template>
    ...
    <div v-else-if="overlayChat" class="overlayChat" @click="closeReplyMessage()">
    ...
    <div
            ...
            v-loading="loadingConversations || loadingMessage"
    >
        ...
        <ConversationStatusBar
                ...
                :visibleSearch.sync="visibleSearch"
                :searchString.sync="searchString"
        />
        <div
                class="app--conversation--drop--wrap"
        >Перетащите сюда фотографии, чтобы отправить их</div>
        ...
        >
            ...
            <div
                v-if="loadingMessagePage"
                v-loading="loadingMessagePage"
                class="message-page-preloader"
            ></div>
        ...
        >
            <app-typing :conversation-id="currentConversationId"></app-typing>
        </div>
...
<script>
    ...
    export default {
           ...
           /* activeWindow(v){
            ...
            async messages(v) {
                ...
                if (this.$refs[this.messageIdToScrollPage]) {
                    ...
                    let msg = this.$refs[this.messageIdToScrollPage][0]

                    if (msg) {
                        let conversationField = this.$refs.conversationField
            ...
            messageForCenterId(v) {
                if (v) {
                    ...
                    let feed = this.$refs.conversationField
                    let msg = this.$refs['message' + v]
        ...
        computed: {
            ...
            conversation() {
                let conversation = this.$store.getters["chat/conversation/conversations"].filter(item => item.id == this.currentConversationId)
            ...
            messages() {
                ...
                if (this.currentConversationId) {
                    messages = this.$store.getters["chat/message/messages"][this.currentConversationId]
                    if (this.topicFilter) {
                        let newMessages = {}
        ...
        methods: {
            searchByFilterString(searchable) {
                ...
                if (searchable) {
                    ...
                    for (let item in this.messages) {
                        ...
                        if (this.messages[item].parent_message && this.messages[item].parent_message.attachments && this.messages[item].parent_message.attachments.length) {

                            if (this.messages[item].parent_message.attachments[0].type === 'message/text') {
                        ...
                        } else if (this.messages[item].attachments && this.messages[item].attachments.length) {
                ...
                for (let i = 0; i < this.files.length; i++) {
                    if (/\.(jpe?g|png|gif)$/i.test(this.files[i].name)) {
                        ...
                        reader.addEventListener("load", function () {
                            ...
                            if (this.$refs['preview' + parseInt(i)][0].naturalHeight > this.$refs['preview' + parseInt(i)][0].naturalWidth) {
                        ...
                        this.$nextTick(function () {
                            this.$refs['preview' + parseInt(i)][0].src = 'https://nektony.com/wp-content/uploads/2016/10/file.svg';
                            if (this.$refs['preview' + parseInt(i)][0].naturalHeight > this.$refs['preview' + parseInt(i)][0].naturalWidth) {
                                this.$refs['preview' + parseInt(i)][0].height = 260
                                this.$refs['preview' + parseInt(i)][0].style.height = '260px'
                                this.$refs['preview' + parseInt(i)][0].width = 146
            ...
            validateFile() {
                let attachFile = this.$refs.attachFile.files[0];
                if (attachFile) {
            ...
            async updateMessages(messages) {
                ...
                // массовое прочтение сообщений
                    await this.$store.dispatch("chat/message/toRead", this.currentConversationId)
        },
